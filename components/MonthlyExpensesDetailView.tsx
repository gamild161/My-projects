...
import { Expense, DailyLog, Partner } from '../types';
import { Search, Printer, Edit2, Trash2, Calendar, Wallet, Download, X, Save } from 'lucide-react';
import { printExpenseReport } from '../utils/printer';
...
const MonthlyExpensesDetailView: React.FC<MonthlyExpensesDetailViewProps> = ({ 
  ...
  // تجميع كافة المصاريف من المصدرين
  const allExpenses = useMemo(() => {
    const list: Array<Expense & { source: 'current' | number }> = [];
    
    // إضافة المصاريف الحالية
    currentExpenses.forEach(exp => {
      list.push({ ...exp, source: 'current' });
    });
    
    // إضافة المصاريف المؤرشفة
    dailyLogs.forEach((log, logIdx) => {
      log.expenses.forEach(exp => {
        list.push({ ...exp, source: logIdx });
      });
    });
  ...
  const handleUpdate = (e: React.FormEvent) => {
    ...
    if (source === 'current') {
      ...
      // إعادة احتساب الربح الصافي لليومية المؤرشفة
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-center gap-6 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
        <div className="flex items-center gap-4 w-full md:w-auto">
          ...
          <div>
            <h3 className="text-xl font-bold text-gray-800">جرد المصاريف التفصيلي</h3>
            <p className="text-xs text-gray-400">تحليل وتدقيق كافة بنود الصرف الشهري</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-4 w-full md:w-auto">
          <div className="relative flex-1 min-w-[200px]">
            <input 
              type="text" 
              placeholder="ابحث عن مبلغ أو بيان..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pr-10 pl-4 py-2 bg-gray-50 border border-gray-100 rounded-xl focus:ring-2 focus:ring-red-500 outline-none text-sm"
            />
          ...
          <button 
            onClick={() => printExpenseReport(allExpenses, selectedMonth, totalAmount)}
            className="bg-slate-800 hover:bg-slate-900 text-white px-5 py-2 rounded-xl text-sm font-bold flex items-center gap-2 transition-all shadow-lg"
          >
            <Printer size={16} />
            طباعة الكشف
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm text-center">
          <p className="text-gray-400 text-xs font-bold mb-1 uppercase tracking-wider">الحالة الضريبية</p>
          ...
          <table className="w-full text-right">
            <thead className="bg-gray-50 text-gray-500 text-xs uppercase">
              <tr>
                ...
                <th className="px-6 py-4 font-black">البيان (نوع الصرف)</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {allExpenses.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-gray-400 italic">لا توجد مصاريف مطابقة في هذه الفترة</td>
                </tr>
              ) : (
                allExpenses.map((exp) => (
                  <tr key={exp.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-gray-700">{exp.date}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`text-[10px] font-bold px-2 py-1 rounded-full ${exp.source === 'current' ? 'bg-emerald-50 text-emerald-600' : 'bg-blue-50 text-blue-600'}`}>
                        {exp.source === 'current' ? 'يومية حالية' : 'يومية مؤرشفة'}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-sm font-medium">{exp.expenseType}</td>
                    <td className="px-6 py-4">
                      <span className="font-black text-red-600">{exp.amount.toLocaleString()} ر.س</span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-center gap-2">
                        <button 
                          onClick={() => setEditingItem({ id: exp.id, source: exp.source, data: { ...exp } })}
                          className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all"
                        >
                          <Edit2 size={16} />
                        </button>
                        <button 
                          onClick={() => setConfirmDelete({ id: exp.id, source: exp.source })}
        ...
        <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-fade-in">
            <div className="bg-red-600 p-6 text-white flex justify-between items-center">
              <h3 className="font-bold text-lg">تعديل بند مصروف</h3>
              <button onClick={() => setEditingItem(null)}><X size={24} /></button>
            </div>
            <form onSubmit={handleUpdate} className="p-8 space-y-6">
              <div className="space-y-4">
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase">التاريخ</label>
                  <input 
                    type="date" 
                    value={editingItem.data.date} 
                    onChange={e => setEditingItem({...editingItem, data: {...editingItem.data, date: e.target.value}})}
                    className="w-full px-4 py-3 border border-gray-100 rounded-xl font-bold outline-none focus:ring-2 focus:ring-red-500" 
                  />
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase">البيان</label>
                  <input 
                    type="text" 
                    value={editingItem.data.expenseType} 
                    onChange={e => setEditingItem({...editingItem, data: {...editingItem.data, expenseType: e.target.value}})}
                    className="w-full px-4 py-3 border border-gray-100 rounded-xl font-medium outline-none focus:ring-2 focus:ring-red-500" 
                  />
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase">المبلغ</label>
                  <input 
                    type="number" 
                    ...
                    className="w-full px-4 py-3 border border-gray-100 rounded-xl font-black text-red-600 text-xl outline-none focus:ring-2 focus:ring-red-500" 
                  />
                </div>
              </div>
              <button type="submit" className="w-full bg-red-600 text-white py-4 rounded-2xl font-bold shadow-xl shadow-red-100 flex items-center justify-center gap-2">
      ...
      {confirmDelete && (
        <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-sm shadow-2xl p-8 text-center animate-fade-in">
            ...
            <h3 className="text-xl font-bold text-gray-800 mb-2">تأكيد الحذف النهائي</h3>
            <p className="text-gray-500 text-sm mb-8">سيتم حذف هذا المصرف من السجلات وإعادة احتساب الأرباح تلقائياً. هل أنت متأكد؟</p>
